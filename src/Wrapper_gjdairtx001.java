...
public class Wrapper_gjdairtx001 implements QunarCrawler {
	...
	public static void main(String[] args) {
		...
		Wrapper_gjdairtx001 wrap = new Wrapper_gjdairtx001();
...
//			httpClient.getParams().setCookiePolicy(CookiePolicy.BROWSER_COMPATIBILITY);
			...
			NameValuePair[] names = {
					...
					new NameValuePair("B_DATE_2",date1),
					new NameValuePair("DATE_RETOUR",date2),
	...
	public ProcessResultInfo process(String html, FlightSearchParam param) {
		...
		try{
			...
			JSONObject boundJson =  tabObj.getJSONArray("list_proposed_bound").getJSONObject(0);
			JSONArray flightArray = boundJson.getJSONArray("list_flight");
			for (int i = 0; i < flightArray.size(); i++){
				JSONObject flightInfo = flightArray.getJSONObject(i);
				OneWayFlightInfo flight = new OneWayFlightInfo();
				FlightDetail flightDetail = new FlightDetail();
				List<FlightSegement> segs = new ArrayList<FlightSegement>();
				flightDetail.setWrapperid("gjdairtx001");
			...
			JSONObject[] priceArray = getPriceArray(flightArray.size(), tabObj); 
	...
	public BookingResult getBookingInfo(FlightSearchParam param) {
		...
		map.put("B_DATE_2",date1);
		map.put("DATE_RETOUR",date2);
	...
	private JSONObject[] getPriceArray(int size, JSONObject boundJson) {
		JSONObject[] priceArray = new JSONObject[size]; 
		...
		for(int i=0; i< recArray.size(); i++){
			...
			for(int j=0; j< boundArray.size(); j++){
				JSONObject boundObj = boundArray.getJSONObject(j);
				JSONArray flightArray = boundObj.getJSONArray("list_flight");
				for (int k=0; k<flightArray.size(); k++){
					JSONObject flightObj = flightArray.getJSONObject(k);
					int flightId = flightObj.getIntValue("flight_id");
					if (priceArray[flightId] == null){
						priceArray[flightId] = recObj; 
					}
					else	if (priceArray[flightId].getFloatValue("price") > recObj.getFloatValue("price")){
						priceArray[flightId] = recObj;
					}
				}
			}
